...
| `MATH_UNITS` | 12 册 / 96 条 | 本册**完整**单元清单（人教版 2024 新版），见下 |

### `MATH_UNITS` —— 「本册有哪些单元」（**正向**）

`CONTENT[..]['math'][册]` 里每个题型带一个 `unit` 字段，但那个字段是
**「题型 → 单元」的逆向映射，天然残缺**：人教版四下教材有 10 个单元，
题型只覆盖到 3 个（缺的是认识 / 概念 / 图形类，本地生成器本来就出不了）。
所以它答不了「本册按教材进度有哪些内容」，而 AI 出题恰恰需要这个
才能让应用题的情境与数据贴合本册。

```
MATH_UNITS: { "_meta": {...}, "renjiao": { "4": { "上": ["一 万以上数的认识", ...] } } }
```

- 形状 `{版本:{年级:{册:[单元名]}}}`，`*` 开头的是「综合与实践（项目式）」板块。
- **与题型白名单的分工必须写清，否则两条约束会互相顶**：
  单元里有「认识立体图形」「图形的运动」这类本地生成器出不了的单元，
  若被当成题型清单用就是直接超纲。所以 AI 提示词里的顺序是
  「先说能出什么（题型白名单）→ 再说情境落在哪（单元清单）」，
  并显式写明「绝不可因此引入上面清单之外的题型或运算」。
- **版本对齐**：`_meta.alignment` 声明哪些版本的数学课本实际用人教版
  （`renjiao` / `tongbiao` / `waiyanYQ` / `waiyanSQ`）。
  **冀教版不在其中** —— 它用的是冀教版数学教材，套用人教版单元名会说错。
  这层对齐关系放在**数据里**而不是写进代码：它是关于教材的事实，
  改数据比改代码安全。
- **唯一取用口 `AppData.mathUnitsFor(版本, 年级, 册)`**；
  版本不在 `alignment` 里、或年级 / 册不存在时返回**空表、不抛错** ——
  调用方（提示词、面板）必须能安全回退到「只用题型清单」。
- **数据依据人教版 2024 新版**（2022 版课标修订）。滚动启用：
  2024 秋一上 / 2025 春一下 / 2025 秋二上三上 / 2026 春二下三下 /
  2026 秋四上五上六上 / 2027 春全部下册。也就是说**当前（2026 秋）
  一上～六上在用的都已经换成新版**，旧版数据没有用户。
...
| `div3x2` 除数是两位数 | waiyanSQ/hebei/tongbiao 4 上（renjiao 见下方遗留项） |
...
AppData（单例，lib/data/app_data.dart）
  ...
  ├─ mathUnitsFor(ver, grade, vol)  ★ 本册**完整**单元清单（正向），见 §5
  ├─ mathUnitsAligned       哪些版本的数学课本实际用人教版（取自数据的 _meta）
...
5. **`VERSION_SUPPORT` 的 `cally` 一个键管两件事**（练字帖 + 语文），
   改动它会影响两张卡片。
6. **遗留项（未处理）：数学题型在 `CONTENT` 里的位置与 `unit` 文案仍是旧版口径。**
   `MATH_UNITS` 是新版（2024），两者在数学面板上并排显示，用户能看出不一致。
   已核实的两处：

   | 题型 | `CONTENT` 标着 | 2024 新版实际 |
   |---|---|---|
   | `div3x2` 除数是两位数的除法 | 人教版**四上 第6单元** | 人教版**四下第一单元** |
   | `mul3x2` 三位数乘两位数 | 人教版四上 **第4单元** | 新版叫「三 多位数乘两位数」（第 3 单元） |

   注意这不只是显示问题：题型挂在哪个 `CONTENT[..]['math'][册]` 下，
   决定了本地生成器在**哪一册**出这个题型。要处理就得按新版目录逐个校
   数学题型的年级 / 册归属，需要单独一轮有来源的校对，别顺手改。
