...
版面都不同：课题可能是 `第1课 认识新同学`，也可能是 `2会唱歌的石头`（无「第」「课」）；
...
- **目录里页码不增的连续条目按「父 + 子」合并**：《三首童谣》下挂着
  `露珠 / 会走路的树 / 数星星的孩子`，四条书页都是 11；《两首小诗》下有
  `溪边 / 村晚`。拆开的话后者 `end < start`，会切出空课。
  「诗词赏读」下十首诗页码依次递增，则照常各自成课。
...
**切不出课时的诊断：三种原因的处置正好相反，必须先分清。**
`profileLanguage()` 统计清洗后正文的**非空白字符**里汉字与阿拉伯数字的占比，
`noLessonMessage()` 据此给三套完全不同的话术：

| 判据 | 结论 | 建议 |
|---|---|---|
| `enoughText == false`（< 300 字符） | 不下结论，可能是扫描图片版 | 先 OCR，或走「拍照导入」 |
| 汉字占比 < 30%（`kChineseHanRatio`） | 外文教材 | 换书，或走「拍照导入」（视觉识别不受语种限制）。**OCR 没用** |
| 数字占比 ≥ 15%（`kMathDigitRatio`） | 数学教材 | **别在 PDF 上继续折腾**，去「数学」面板按单元出题 |

实测四本真实教材（汉字占比 / 数字占比 / 切出的课数）：
...
| 语文六年级下册 | 83.0% | 1.5% | 46 |
| **数学四年级下册** | **52.6%** | **28.0%** | **0** |
| 英语（三年级起点）六年级下册 | 2.3% | 2.1% | 0 |

第三行是关键：数学教材的**汉字占比完全正常**，单看汉字占比根本分不出来，
而它同样一节课都切不出来 —— 不单独判就会给用户「请确认这本 PDF 带文本层」
这种与事实相反的建议（实测英语那本 81 页、35548 字符全部提取成功却 0 课，
也是同一个坑）。数字占比 28% 对上其余三本最高 5.2%，阈值取 15% 两边都有余量。

文案**用对话框而不是 SnackBar 弹出**：三四行的诊断，SnackBar 6 秒就消失，
用户还没读到关键那句 —— 而「让用户读到」正是它存在的理由。
另外，「整页读不出文字」的页号**三条支路都必须带上**（见 `unreadablePages`
的契约），静默不提会让人以为整本书都读到了。

⚠️ **已知局限（两条，都是「提取得到但切不出来」）**：

1. 部分教材把数字做成了**图片**（`我一般〔9〕点睡觉`），文本层里根本不存在，
   且注音音节数仍等于汉字数，评分抓不到 —— 只能靠视觉兜底 + 人工确认。
2. **英文教材的拼行是另一套活儿。** 清洗器是按中文「一字一词」写的，同行词直接
   `join()`；英文词间有空格，这样拼出来是 `Slowandsteadywinstherace`。
   已实测的可行方向：英文词间隙 82% 落在 1.5–6pt（p50 = 3.00pt），**可以靠几何间隙
   重建空格**。但**不能无条件按间隙补空格** —— 中文书的原词间隙分布是乱的
   （语文 p50 = 0.28pt、德法 p50 = 5.35pt，1.5–6pt 区间里德法有 106 个），
   补了就误插空格。所以顺序必须是「先判语种 → 再决定拼行策略」。
   要做英文教材，第一步是把这个语种开关接进 `pdf_page_cleaner`，
   而不是先调结构识别。
...
- **数学的「本册有哪些单元」与「能出哪些题型」是两张不同的表，方向相反。**
  题型里的 `unit` 是「题型 → 单元」的**逆向**映射，天然残缺（人教版四下 10 个
  单元，题型只覆盖 3 个）。正向的完整清单是 `MATH_UNITS`，
  唯一取用口 `AppData.mathUnitsFor(版本, 年级, 册)`，越界返回空表不抛错。
  它**只用来让 AI 应用题的情境贴合本册进度，绝不能当成题型清单** ——
  单元里有「认识立体图形」「图形的运动」这类本地生成器出不了的，
  拿来扩题型就是直接超纲。细节见 DATA_MODEL.md §5。
- **`CONTENT[..]['math'][册]` 里题型的「位置 + `unit` 文案」都是旧版教材口径，
  而 `MATH_UNITS` 是 2024 新版，两者在面板上并排显示，可能对不上。**
  实例：`div3x2`（除数是两位数的除法）标着「人教版四上 第6单元」，
  但 2024 新版把它编在**四下第一单元**；`mul3x2` 标「四上 第4单元」，
  新版是「三 多位数乘两位数」。**这是尚未处理的遗留项**，
  要做就得按新版目录逐个校 `CONTENT` 的数学题型归属，见 DATA_MODEL.md §10。
...
- `textbook_structurer_test.dart` 教材结构识别（合成小书：点线目录、子条目合并、
  「目录对不上正文时退回标题带」、分段续接逐字一致、语种判定与数学教材判定）
- `pdf_import_diagnosis_test.dart` 「切不出课」的三套诊断文案不串味
  （扫描版 / 外文 / 数学各说什么、不说什么，用实测占比当输入）

⚠️ 改教材 PDF 相关逻辑后，**只跑单元测试不够** —— 合成夹具的版面参数是我们
自己写进去的，必然会顺应实现。真实样本在 `~/samples/`（`_real_textbook.pdf`
《道德与法治》一年级上册、`_yuwen6x.pdf` 《语文》六年级下册、
`_eng6x.pdf` 英语三年级起点六年级下册），
改完务必用它们各跑一遍，并检查「分段导入与整本导入逐字一致」。
语种判定改动要三本都验：两本中文**不能被误判**，英语那本要判为非中文。
