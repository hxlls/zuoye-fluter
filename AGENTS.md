...
**语种判定：导入只认中文教材。** `profileLanguage()` 统计清洗后正文的汉字占比，
`< kChineseHanRatio`（30%）且文本量 ≥ 300 字符时判为外文教材，报明确诊断而不是
「请确认这本 PDF 带文本层」。实测三本：德法一年级上册 77.1%、语文六年级下册 83.0%、
英语（三年级起点）六年级下册 **2.3%** —— 阈值两边余量都极大。判据存在的理由是：
英语那本 81 页、35548 字符**全部提取成功**，但一节课都切不出来；
原来那句提示会把用户引向「去做 OCR」这条完全错的路。
